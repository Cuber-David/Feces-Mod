...
import K.content.extend.util.MathUtil;
import arc.Core;
...
@HeadlessDisabled
//render a screen-space statuseffect for StatusEffect.
public class StatusRenderer {
    ...
    public void registerStatusEffects() {
        register(statuseffect.infinitude, 80, (warmup, unit, status) -> {
            Color from = Color.valueOf("4b197e"), to = Color.valueOf("7a007a");
        ...
        register(statuseffect.none, 60, NHShaders.statusXWave, (warmup, unit, status) -> {
            int step = 36;
                    ...
                    for (int k = 0; k < 6; k++) {
                        ...
                        if (!(Tmp.v2.x - rad > right() || Tmp.v2.x + rad < left() || Tmp.v2.y - dst > top() || Tmp.v2.y + dst < bottom())) {
                            ...
                            if (rand.chance(chance)) {
                                ...
                                Tmp.c1.set(Color.white).lerp(Color.blue, innerWarmup * MathUtil.timeValue(0.2f, 0.6f, 1.5f, rand.random(360)));
                                Draw.color(Tmp.c1);
        ...
        //scannerDown
        register(statuseffect.none, 100, (warmup, unit, status) -> {
            Color from = Pal.heal, to = Color.white;
        ...
        //emp123
        register(statuseffect.none, 120, (warmup, unit, status) -> {
            ScreenShaderDrawer.drawDisplaceGlitch = warmup > 0.01f;
            NHShaders.displaceGlitch.intensity = warmup * 1f;
        });

        register(statuseffect.none, 130, (warmup, unit, status) -> {
            ScreenShaderDrawer.drawDisplaceGlitch = warmup > 0.01f;
            NHShaders.displaceGlitch.intensity = warmup * 1.6f;
        });

        register(statuseffect.infinitude, 140, (warmup, unit, status) -> {
            ScreenShaderDrawer.drawDisplaceGlitch = warmup > 0.01f;
