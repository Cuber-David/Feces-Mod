...
public class KSFX implements ApplicationListener{
    public static KSFX inst;

    private final Utils.BasicPool<LockMovement> lockPool = new Utils.BasicPool<>(LockMovement::new);
    ...
    private static final Utils.BasicPool<UnitHealPrevention> uhpPool = new Utils.BasicPool<>(UnitHealPrevention::new);
    private static final Utils.BasicPool<BuildingHealPrevention> bhpPool = new Utils.BasicPool<>(BuildingHealPrevention::new);

    private static float[] bulletDps, unitDps;

    float impFrameTime = 0f;
    Seq<ImpactFrameDrawer> impFrameEntities = new Seq<>();
    Utils.BasicPool<ImpactFrameDrawer> impFramePool = new Utils.BasicPool<>(ImpactFrameDrawer::new);
    Floatp trueDelta = () -> {
