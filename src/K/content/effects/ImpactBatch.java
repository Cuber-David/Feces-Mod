...
public class ImpactBatch extends SpriteBatch{
    ...
    protected void draw(Texture texture, float[] spriteVertices, int offset, int count){
        ...
        int verticesLength = spriteVertices.length;
        ...
        System.arraycopy(spriteVertices, offset, spriteVertices, idx, copyCount);
        idx += copyCount;
        count -= copyCount;
        while(count > 0){
            offset += copyCount;
            ...
            System.arraycopy(spriteVertices, offset, spriteVertices, 0, copyCount);
            idx += copyCount;
            count -= copyCount;
        }
    ...
    protected void draw(TextureRegion region, float x, float y, float originX, float originY, float width, float height, float rotation){
        ...
        if(texture != lastTexture){
            switchTexture(texture);
        }else if(idx == mesh.vertexSize || (heavyShader && idx >= SPRITE_SIZE)){
            flush();
        }

        float[] vertices = this.svt;
        int idx = this.idx;
