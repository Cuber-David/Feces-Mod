...
public class KPlanetGenerator extends PlanetGenerator implements WorldGenerator{
    ...
    Block[][] arr =
            {
                    {Blocks.mud,Blocks.dirt, blocks.groundfecalwater,blocks.groundurine},
            };
    ...
    public boolean allowNumberedLaunch(Sector s){
        return s.hasBase() && !s.isAttacked() && (s.info.bestCoreType.size >= 1 || s.isBeingPlayed() && state.rules.defaultTeam.cores().contains(b -> b.block.size >= 1));
    }
    ...
    public void getLockedText(Sector hovered, StringBuilder out){
        if((hovered.preset == null || !hovered.preset.requireUnlock) && hovered.near().contains(Sector::hasBase)){
            if(hovered.isShielded()){
                ...
                out.append("[red]").append(Iconc.cancel).append("[]").append(blocks.fecescore.emoji()).append(Core.bundle.get("sector.foundationrequired"));
            }
        }else{
    ...
    public void getColor(Vec3 position, Color out){
        ...
        if(block == Blocks.mud) block = Blocks.dirt;
    ...
    protected void generate(){
                        ...
                        for(int y = -rad; y <= rad; y++){
                            ...
                            if(Structs.inBounds(wx, wy, width, height) && Mathf.within(x, y, rad)){
                                ...
                                if(Mathf.within(x, y, rad - 1) && !other.floor().isLiquid){
                                    ...
                                    other.setFloor((Floor)(floor == Blocks.dirt || floor == Blocks.mud ? blocks.groundurine : blocks.groundfecalwater));
                                }
                            }
                        }
                    }
        ...
        for(int i = 0; i < tlen; i++){
            ...
            if(tile.block() == Blocks.air){
                total ++;
                if(tile.floor().liquidDrop == liquids.urine){
                    waters ++;
                }
            }
        }
        ...
        pass((x, y) -> {
            ...
            if(value > 0.17f && !Mathf.within(x, y, fspawn.x, fspawn.y, 12 + rrscl)){
                ...
                if(!(floor == Blocks.mud || floor == Blocks.dirt || floor.asFloor().isLiquid)){
                    floor = spore ?
                            (deep ? blocks.groundurine : blocks.groundfecalwater) :
                            (deep ? Blocks.water :
                                    (floor == Blocks.mud || floor == Blocks.dirt ? blocks.groundurine : blocks.groundfecalwater));
                }
            }
        });
        ...
        pass((x, y) -> {
            ...
            if(floor.asFloor().isLiquid && floor.asFloor().shallow){
                ...
                floor = floor == blocks.groundfecalwater ? blocks.groundurine : blocks.groundurine;
            }
        });
        ...
        if(naval){
            ...
            pass((x, y) -> {
                if(floor.asFloor().isLiquid && !floor.asFloor().isDeep() && !floor.asFloor().shallow){
                    ...
                    floor = floor == blocks.groundurine ? blocks.groundfecalwater : blocks.groundurine;
                }
            });
        }

        Seq<Block> ores = Seq.with(blocks.groundfeces, Blocks.oreLead);
        ...
        pass((x, y) -> {
            ...
            if(floor == Blocks.hotrock){
                ...
                if(noise > 0.67f && !roomseq.contains(e -> Mathf.within(x, y, e.x, e.y, 14))){
                    if(noise > 0.72f){
                        floor = noise > 0.78f ? blocks.groundurine : (floor == Blocks.mud ? blocks.groundurine : blocks.groundfecalwater);
                    }else{
                        floor = (floor == Blocks.mud ? floor : Blocks.dirt);
                    }
                }
            }
            ...
            if(rand.chance(0.0075)){
                ...
                if(any && ((block == Blocks.dirtWall || block == Blocks.dirtWall) || (all && block == Blocks.air && floor == Blocks.mud && rand.chance(0.03)))){
                    block = rand.chance(0.5) ? Blocks.dirt : Blocks.dirt;
                }
            }
        ...
        Schematics.placeLoadout(loadouts.basiccore, spawn.x, spawn.y);
