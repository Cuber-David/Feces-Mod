...
public class UltFire extends Fire {
    ...
    public static final Effect remove = new Effect(70f, e -> {
        ...
        Drawf.light(e.x, e.y, 50f + Mathf.absin(5f, 5f), Color.white, 0.6f * e.fout());
    });
    ...
    public static void load() {
        for (int i = 0; i < 40; ++i) {
            ultRegion[i] = Core.atlas.find(KMod.name("ult-fire-") + i);
        }
    }
    ...
    public void draw() {
        ...
        Draw.reset();
        Drawf.light(x, y, 50.0F + Mathf.absin(5.0F, 5.0F), Color.white, 0.6F * Mathf.clamp(warmup / 20.0F));
    }
