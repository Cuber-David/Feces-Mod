...
import K.content.extend.Bullets.EffectBulletType;
 ...
 *
 * @author Yuria
 * @implNote The method implements the generation of random lightning statuseffect <b>from point to point</b> and complete certain action at <b>target point</b> through {@link Cons}.<p>
...
public class PosLightning {
    ...
    public static final Effect posLightning = (new Effect(lifetime, 1200.0f, e -> {
        if (!(e.data instanceof K.content.extend.util.Vec2Seq)) return;
    ...
    //create lightning statuseffect.
    public static void createBoltEffect(Color color, float width, Vec2Seq vets) {
            vets.each(((x, y) -> {
                if (Mathf.chance(0.0855)) Fx.sparkExplosion.at(x, y, rand.random(2f + width, 4f + width), color);
            }));
