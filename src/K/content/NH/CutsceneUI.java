...
import static K.content.NH.NHVars.cutscene;
import static K.content.NH.NHVars.cutsceneUI;
...
public class CutsceneUI {
    ...
    public void init() {
        ...
        buildOverlay();
        //curtain ui, for cutscene curtain and fade in/fade out statuseffect
        buildCurtain();
    ...
    private void updatePosition() {
        ...
        if (Vars.mobile) {
            textTable.update(() -> {
                textTable.setHeight(NHRenderer.height * textTableScl());
                textTable.setWidth(NHRenderer.width);
                textTable.setPosition(0, 0);
            });
            infoTable.update(() -> {
                infoTable.setHeight(NHRenderer.height * infoTableScl());
                infoTable.setWidth(NHRenderer.width);
                infoTable.setPosition(0, 0);
            });
        } else {
            textTable.update(() -> {
                textTable.setSize(Scl.scl(NHRenderer.width * 0.65f), Scl.scl(NHRenderer.height * 0.1f));
                textTable.setPosition((NHRenderer.width - textTable.getWidth()) / 2, NHRenderer.height * 0.14f);
            });
            infoTable.update(() -> {
                infoTable.setSize(Scl.scl(NHRenderer.width * 0.25f), Scl.scl(NHRenderer.height * 0.1f));
                infoTable.setPosition(NHRenderer.width * 0.05f, NHRenderer.height * 0.1f);
            });
        }
    }
    ...
    public void updateLetterboxTextPosition() {
        ...
        float progress = curtainInterp.apply(curtainProgress);
        float bottomH = NHRenderer.height * curtainBottomScl() * progress;
        float padX = NHRenderer.width * 0.06f;
        float padY = Math.max(8f, bottomH * 0.18f);
        float areaW = Math.max(1f, NHRenderer.width - padX * 2f);
    ...
    public void clearMarkers(Object o) {
        clearMarkers(null);
    }
